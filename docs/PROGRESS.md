...
- **2026-07-16 — Footer positioning/compliance copy updated.** Per user request: the footer tagline changed from "Authorized Real Estate Channel Partner" to "Authorized Marketing Partner"; the Compliance block now adds "For Authorized Real Estate Channel Partner" above the MahaRERA line, and the previously-placeholder MahaRERA number is now confirmed as **A51700015804**; the disclaimer paragraph was reworded to "Fusion Ace acts as a marketing partner for authorized real estate channel partner, MahaRERA Reg. No: A51700015804." All changes in `components/Footer.tsx`. Note: `app/terms-and-conditions/page.tsx` and `components/WhyUs.tsx` still use the older "channel partner" phrasing in unrelated copy — left untouched since the user's request was scoped to the footer/disclaimer, but flagged for consistency review.
...
- **2026-07-20 — Wired the lead form to actually send email.** The form previously only did `console.log` and showed a fake success state — nothing was ever sent to `hello@fusionace.in`, which is why enquiries never showed up in Hostinger mail. Added `app/api/enquire/route.ts` (Next.js Route Handler) using `nodemailer` over Hostinger SMTP (`smtp.hostinger.com:465`), reading credentials from env vars (`SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS`, `MAIL_TO`) so nothing sensitive is hardcoded or committed (`.env.local` is gitignored via the existing `.env*` rule). `components/LeadForm.tsx` now `fetch`es this endpoint on submit, with a loading state on the button and an inline error message if sending fails, instead of always showing success. Verified locally: invalid payloads are rejected server-side with a 400, and a valid payload correctly reaches Hostinger's SMTP server and fails with `535 5.7.8 authentication failed` because `.env.local` still holds a placeholder password — **user needs to put the real `hello@fusionace.in` mailbox password into `.env.local`'s `SMTP_PASS`** (and set the same 4 env vars in the production hosting environment) before this will actually deliver mail.
